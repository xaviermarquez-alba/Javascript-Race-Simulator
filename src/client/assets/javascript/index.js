...
async function handleCreateRace() {
    ...
    try {
        await createRace(player_id, track_id).then((race) => {
            // (race.ID - 1) in order to keep consistenci with API
            saveStore(store, { race_id: race.ID - 1 })
            // render starting UI
        ...
        const raceInterval = setInterval(() => {
            getRace(raceID)
                ...
                .catch((err) => {
                    ...
                    throw new Error(`Error runRace ${err.message}`)
                })
        }, 500)
    }).catch((err) => {
        throw new Error(`Error runRace ${err.message}`)
...
async function runCountdown() {
    try {
        ...
        throw new Error(`Error runCountdown ${err.message}`)
    }
}
...
function handleSelectPodRacer(target) {
    ...
    saveStore(store, { race_id: target.id })
}
...
function handleAccelerate() {
    ...
    accelerate(store.race_id).catch((err) =>
        console.log(`Error handleAccelerate ${err.message}`)
    )
}
...
function getTracks() {
    return fetch(`${SERVER}/api/tracks`)
        ...
        .catch((err) => {
            throw new Error(`Error: getTracks ${err.message}`)
        })
}
...
function getRacers() {
    return fetch(`${SERVER}/api/cars`)
        .then((cars) => cars.json())
        .catch((err) => {
            throw new Error(`Error: getRacers ${err.message}`)
        })
}
...
function createRace(player_id, track_id) {
    ...
    return fetch(`${SERVER}/api/races`, {
        ...
        .then((res) => res.json())
        .catch((err) => {
            throw new Error(`Error: createRace ${err.message}`)
        })
}
...
async function getRace(id) {
    return await fetch(`${SERVER}/api/races/${id}`)
        .then((data) => data.json())
        .catch((err) => {
            throw new Error(`Error: getRace ${err.message}`)
        })
}
...
async function accelerate(id) {
    return await fetch(`${SERVER}/api/races/${id}/accelerate`, {
        ...
        throw new Error(`Error: accelerate ${err.message}`)
    })
}
